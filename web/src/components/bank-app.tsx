  ...
  return (
    <div className="app-view-head">
      ...
      <div className="app-head-actions">
        ...
        <button className={`privacy-pill ${privateMode ? "active" : ""}`} onClick={() => setPrivateMode(!privateMode)} aria-pressed={privateMode}>∅ {privateMode ? "PRIVATE" : "STANDARD"}</button>
      </div>
    </div>
  );
}
      ...
      <div className="bank-shell">
        <aside className="bank-sidebar">
          <a href="/" className="app-logo" aria-label="Booty Bank home">
            <img src="/brand/bootybank-mark.svg" alt="" width="40" height="40" />
          </a>
        ...
        <div className="bank-content">
          ...
          {view === "home" && (
            <div className="app-dashboard">
              <div className="balance-hero">
                <div className="balance-copy"><span>TOTAL BALANCE</span><strong>{privateMode ? "••••••" : "$28,491.08"}</strong><small>{privateMode ? "AMOUNT HIDDEN" : "+$6,142.10 THIS MONTH / +27.4%"}</small></div>
              ...
              <div className="dashboard-grid">
                ...
                <article className="payout-card">
                  ...
                  <strong>{privateMode ? "••••••" : "$5,120–$5,640"}</strong>
                  <div className="split-preview"><span>TAX 30%</span><span>OPERATING 15%</span><span>YOURS 55%</span></div>
                  <button className="inline-action" onClick={() => setView("creator")}>OPEN CREATOR ↗</button>
                </article>
          ...
          {view === "payments" && (
            <div className="app-page">
              <div className="page-lead"><div><span>MOVE MONEY</span><h4>SEND MONEY.</h4></div><button className="primary-app-button" onClick={() => setAction("send")}>NEW PAYMENT +</button></div>
              ...
              <div className="payment-grid">
                ...
                <article className="upcoming-list"><div className="app-card-head"><h4>UPCOMING</h4><button onClick={() => setCompleted("SCHEDULE OPEN")}>MANAGE</button></div><div><span>AUG 18</span><b>STUDIO RENT</b><strong>{privateMode ? "••••" : "−$1,250"}</strong></div><div><span>AUG 21</span><b>HEALTH INSURANCE</b><strong>{privateMode ? "••••" : "−$390"}</strong></div><div><span>AUG 25</span><b>RECURRING INVEST</b><strong>{privateMode ? "••••" : "−$500"}</strong></div><button className="inline-action" onClick={() => setCompleted("PAYMENT LINK COPIED")}>CREATE PAYMENT LINK ↗</button></article>
              </div>
            </div>
          )}
          ...
          {view === "cards" && (
            <div className="app-page">
              <div className="page-lead"><div><span>4 ACTIVE CARDS</span><h4>YOUR CARDS.</h4></div><button className="primary-app-button" onClick={() => setCompleted("NEW CARD FLOW OPEN")}>NEW CARD +</button></div>
          ...
          {view === "plan" && (
            <div className="app-page">
              <div className="page-lead"><div><span>THIS MONTH</span><h4>PAYOUT RULES.</h4></div><button className="primary-app-button" onClick={() => setCompleted("NEW POCKET OPEN")}>NEW POCKET +</button></div>
              <div className="plan-grid"><article className="spend-ring"><div className="app-card-head"><h4>SPENDING</h4><span>AUGUST</span></div><div className="ring"><span><b>{privateMode ? "••••" : "$5,842"}</b><small>{privateMode ? "AMOUNT HIDDEN" : "OF $8,000"}</small></span></div><div className="legend"><span><i />LIVING 38%</span><span><i />BUSINESS 34%</span><span><i />OTHER 28%</span></div></article><article className="rules-list"><div className="app-card-head"><h4>PAYOUT RULES</h4><Toggle checked={autoSplit} onChange={() => setAutoSplit(!autoSplit)} label="Toggle all payout rules" /></div><div><span><i className="tax" />TAX</span><b>30%</b><small>UNTIL {privateMode ? "••••" : "$25K"}</small></div><div><span><i className="ops" />OPERATING</span><b>15%</b><small>EVERY PAYOUT</small></div><div><span><i className="save" />INVEST</span><b>10%</b><small>MONTHLY</small></div>{advanceActive && <div><span><i className="loan" />ADVANCE</span><b>12%</b><small>OFF THE TOP</small></div>}<button className="inline-action" onClick={() => setCompleted("RULE EDITOR OPEN")}>EDIT AUTOMATION ↗</button></article></div>
              <div className="insight-banner"><span>SPEND SIGNAL</span><p>SOFTWARE SPEND DOWN 18%.</p><button onClick={() => setCompleted("INSIGHT SAVED")}>SAVE THE DIFFERENCE ↗</button></div>
            </div>
          )}

          {view === "wealth" && (
            <div className="app-page">
              <div className="page-lead"><div><span>NET WORTH</span><h4>{privateMode ? "••••••" : "$47,719.08"}</h4></div><button className="primary-app-button" onClick={() => setCompleted("INVEST FLOW OPEN")}>INVEST +</button></div>
              <div className="wealth-grid"><article className="wealth-chart"><div className="app-card-head"><h4>PORTFOLIO</h4><span>{privateMode ? "AMOUNT HIDDEN" : "+$1,422 / 90D"}</span></div><div className="line-chart"><svg viewBox="0 0 500 180" role="img" aria-label="Illustrative portfolio chart"><path d="M0 154 C50 148 78 160 122 126 S190 132 230 99 S295 122 332 72 S412 86 500 18" fill="none" vectorEffect="non-scaling-stroke" /></svg></div><div className="wealth-totals"><div><span>CASH</span><b>{privateMode ? "••••" : "$28,491"}</b></div><div><span>INVESTED</span><b>{privateMode ? "••••" : "$12,428"}</b></div><div><span>FUTURE POCKET</span><b>{privateMode ? "••••" : "$6,800"}</b></div></div></article><article className="recurring-card"><div className="app-card-head"><h4>AUTOPILOT</h4><span>ACTIVE</span></div><strong>{privateMode ? "••••" : "$500"}</strong><p>INVESTED MONTHLY ON THE 25TH.</p><div><span>GLOBAL INDEX</span><b>70%</b></div><div><span>SHORT-TERM BONDS</span><b>30%</b></div><button className="inline-action" onClick={() => setCompleted("AUTOPILOT EDITOR OPEN")}>EDIT AUTOPILOT ↗</button></article></div>
              <p className="app-disclaimer">DEMO ONLY. REGULATED PARTNER REQUIRED. VALUES CAN FALL.</p>
            </div>
          )}

          {view === "creator" && (
            <div className="app-page">
              <div className="page-lead"><div><span>CREATOR ACCOUNT</span><h4>GET PAID. BORROW.</h4></div><button className="primary-app-button" onClick={() => setAction("borrow")}>GET ADVANCE ↗</button></div>
              <div className="creator-grid"><article className="source-card"><div className="app-card-head"><h4>INCOME SOURCES</h4><button onClick={() => setCompleted("SOURCE CONNECTOR OPEN")}>CONNECT +</button></div><div><i>OF</i><span><b>ONLYFANS</b><small>CONNECTED / LAST SYNC 08:42</small></span><strong>{privateMode ? "••••" : "$18,400 / MO"}</strong></div><div><i>PX</i><span><b>PAXUM</b><small>CONNECTED / LAST SYNC 08:40</small></span><strong>{privateMode ? "••••" : "$2,040 / MO"}</strong></div><div className="income-evidence"><span>PRIVATE INCOME DATA</span><b>VERIFIED</b><small>EXPIRES IN 72 DAYS</small></div></article><article className="advance-card"><span>AVAILABLE TO REQUEST</span><strong>{privateMode ? "••••••" : "$24,800"}</strong><p>14 MONTHS VERIFIED. MINIMUM LENDER PACKET.</p><button onClick={() => setAction("borrow")}>REVIEW ADVANCE ↗</button></article></div>
          ...
          {view === "more" && (
            <div className="app-page">
              <div className="page-lead"><div><span>ACCOUNT TOOLS</span><h4>MORE TOOLS.</h4></div></div>
              <div className="more-menu">
                <button onClick={() => setView("plan")}><span>01</span><b>PLAN</b><p>POCKETS + RULES</p><i>↗</i></button>
                <button onClick={() => setView("wealth")}><span>02</span><b>WEALTH</b><p>SAVE + INVEST</p><i>↗</i></button>
                <button onClick={() => setView("privacy")}><span>03</span><b>PRIVACY</b><p>DATA + SECURITY</p><i>↗</i></button>
              </div>
            </div>
          )}
          ...
          {action && (
            <div className="action-backdrop" onClick={closeAction}>
              <div ref={actionDialogRef} tabIndex={-1} className="action-sheet" role="dialog" aria-modal="true" aria-label={`${action} money`} onClick={(event) => event.stopPropagation()}>
                ...
                {action === "borrow" && <><div className="borrow-amount"><span>SAMPLE REQUEST LIMIT</span><b>{privateMode ? "••••••" : "$24,800"}</b><small>14 MONTHS VERIFIED / 88 DATA SCORE</small></div><label><span>REQUEST</span><div className="money-input"><input inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} /><select><option>USD</option></select></div></label><div className="consent-callout"><b>MINIMUM DISCLOSURE</b><p>SCORE + BAND ONLY. NO HANDLE OR PAYOUT HISTORY.</p></div><div className="consent-callout"><b>REPAYS AT SOURCE</b><p>INSTALLMENTS COME OFF THE TOP OF PLATFORM PAYOUTS.</p></div><p className="sheet-legal">DEMO ONLY. NO CREDIT DECISION OR LOAN.</p><button className="sheet-submit" disabled={!Number(amount) || Number(amount) > 24800} onClick={() => { setAdvanceActive(true); complete("LENDER PACKET CREATED"); }}>CREATE LENDER PACKET ↗</button></>}
              </div>
            </div>
          )}
