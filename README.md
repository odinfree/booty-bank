...
The demo supports working interactions for balance redaction, navigation, transfers, exchange, card controls, payout automation, creator advances, and privacy review. Its Starknet rail is real: wallets connect through wallet-standard Get Starknet v6, public balances are read onchain, and AVNU returns live bidirectional quotes across a pinned Mainnet core of STRK, native USDC, ETH, and WBTC. A fresh quote can execute only after Booty Bank independently validates the chain, router, approval, tokens, amount, recipient, minimum output, and fee fields in every builder-returned call. Mainnet swaps use AVNU's pay-gas-in-STRK mode. Sepolia paymaster RPC uses Booty Bank's server-side gas-free proxy. Ready or Xverse can expose the [STRK20 Starknet Wallet API](https://strk20.starknet.io/docs/starknet-wallet-api/overview) for private balances, shield, private transfer, unshield, simulation, proof generation, and submission. Booty Bank never receives a viewing key. Every fiat rail that needs a licensed operator is labelled `PARTNER` or `ROADMAP`.

Mainnet evidence and the required STRK20 transaction-file workflow are documented in [Hackathon Mainnet evidence](docs/HACKATHON-MAINNET-EVIDENCE.md).
...
The Worker exposes read-only AVNU paymaster availability and supported-token metadata at `/api/paymaster`. The API key is a Worker secret and never reaches the browser. Sponsored build and execution are closed until Booty Bank can issue a one-time capability bound to the account, chain, exact calls, and expiry. Mainnet swaps use AVNU directly and the user pays gas in STRK; no project credits are consumed.
